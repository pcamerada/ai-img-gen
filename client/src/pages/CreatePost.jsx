...
import { FormField, Loader, ImagePopup, Alert } from "../components";
...
const CreatePost = () => {
  ...
  const generateImg = async () => {
    if(form.prompt) {
      ...
      openAlert('Please enter a prompt', 'warning');
    }
  }
  ...
  const handleSubmit = async (event) => {
    ...
    if(form.name && form.prompt && form.photo) {
      try {
        setLoading(true);
        const response = await fetch('http://localhost:8080/api/v1/post/upload',{
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
          },
        ...
        await response.json();
        openAlert('Image uploaded successfully!', 'success')
      } catch (error) {
        openAlert(error.message, 'error')
      } finally {
        setLoading(false)
  ...
  const handleClickImage = () => {
    if (form.photo) setIsImagePopupOpen(!isImagePopupOpen)
  }
    ...
    <section className='max-w-7xl mx-auto'>
      ...
      <form className='mt-16 max-w-3xl' onSubmit={handleSubmit}>
        <div className='flex flex-col gap-5'>
          ...
          />
          <div className='relative bg-grey-500 border border-grey-300 
            text-grey-900 text-sm rounded-lg focus:ring-blue-500 
            focus:border-blue-500 w-64 p-3 h-64 flex justify-center
            items-center cursor-pointer' onClick={handleClickImage}>
            {form.photo ? (
      ...
      { (isImagePopupOpen && form.photo != null) && ( <ImagePopup image={form.photo} title={form.prompt} closePopup={handleClickImage}/> ) }
    </section>
  )
}
